...
// coverage_words.js, does `surface` appear ANYWHERE in the candidate list the
...
function main() {
  ...
  const words = require('./coverage_words.js');
  const missing = [];
  const notFirst = [];
  ...
  console.log(`[COVERAGE] present in candidates: ${present}/${n} (${(100 * present / n).toFixed(1)}%)`);
  console.log(`[COVERAGE] of those, at position 0: ${present - notFirst.length}/${present}`);
  if (missing.length) {
