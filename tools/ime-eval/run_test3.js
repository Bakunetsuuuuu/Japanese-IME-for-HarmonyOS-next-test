#!/usr/bin/env node
// Third held-out TEST corpus (corpus_test3.js/accept_test3.js). Per the
...
function main() {
  ...
  const corpus = require('./corpus_test3.js');
  const accept = require('./accept_test3.js');
  ...
  console.log(`[TEST3] strict accuracy : ${strict}/${n} (${(100 * strict / n).toFixed(1)}%)`);
  console.log(`[TEST3] lenient accuracy: ${lenient}/${n} (${(100 * lenient / n).toFixed(1)}%)`);
  if (showMisses) {
