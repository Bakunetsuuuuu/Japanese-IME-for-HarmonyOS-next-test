#!/usr/bin/env node
// Fourth held-out TEST corpus (corpus_test4.js/accept_test4.js). Per the
...
//
// Usage:
//   node tools/ime-eval/run_test3.js            # summary accuracy
//   node tools/ime-eval/run_test3.js --misses   # also print every miss
const fs = require('fs');
...
function main() {
  ...
  const corpus = require('./corpus_test4.js');
  const accept = require('./accept_test4.js');
  ...
  console.log(`[TEST4] strict accuracy : ${strict}/${n} (${(100 * strict / n).toFixed(1)}%)`);
  console.log(`[TEST4] lenient accuracy: ${lenient}/${n} (${(100 * lenient / n).toFixed(1)}%)`);
  if (showMisses) {
