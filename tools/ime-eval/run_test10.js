#!/usr/bin/env node
// Tenth held-out TEST corpus (corpus_test10.js/accept_test10.js), authored
// fresh (not reused from TEST1-9). Doubles as the corpus
// tools/mozc_data/compare_engines.js uses to score the optional mozc-derived
// engine side by side with this default (track A) engine -- see that
// script and tools/mozc_data/README.md.
//
// Usage:
//   node tools/ime-eval/run_test10.js            # summary accuracy
//   node tools/ime-eval/run_test10.js --misses   # also print every miss
const fs = require('fs');
...
function main() {
  ...
  const corpus = require('./corpus_test10.js');
  const accept = require('./accept_test10.js');
  ...
  console.log(`[TEST10] strict accuracy : ${strict}/${n} (${(100 * strict / n).toFixed(1)}%)`);
  console.log(`[TEST10] lenient accuracy: ${lenient}/${n} (${(100 * lenient / n).toFixed(1)}%)`);
  if (showMisses) {
