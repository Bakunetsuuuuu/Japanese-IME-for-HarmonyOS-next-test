...
// sentence conversion. See vocab_test.js for why this exists: corpus_test.js
...
function main() {
  ...
  const words = require('./vocab_test.js');
  let hit = 0;
  const misses = [];
  ...
  console.log(`[VOCAB] coverage: ${hit}/${n} (${(100 * hit / n).toFixed(1)}%)`);
  if (showMisses) {
